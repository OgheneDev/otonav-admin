...
const navGroups = [
  ...
  },
  {
    label: "Riders",
    items: [
      { label: "Verified Riders", icon: ShieldCheck, href: "/dashboard/verified-riders" },
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <div className="flex items-center justify-between px-6 py-6 border-b" style={{ borderColor: "var(--border)" }}>
          <div>
            <h1 className="font-display text-2xl leading-none" style={{ color: "var(--brand-red)" }}>
              OtoNav
            </h1>
            <p className="text-[10px] tracking-[0.2em] uppercase mt-1" style={{ color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>
              Admin Console
            </p>
          </div>
          <button
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-6">
          {navGroups.map((group) => (
            <div key={group.label}>
              <p className="px-3 mb-1.5 text-[10px] font-semibold tracking-[0.15em] uppercase"
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      style={{
                        background: active ? "rgba(0,160,130,0.07)" : "transparent",
                    ...
                    >
                      ...
                      <item.icon size={17} className="shrink-0" style={{ color: active ? "var(--brand-teal)" : "var(--text-muted)" }} />
        ...
        <div className="px-4 py-4 border-t" style={{ borderColor: "var(--border)" }}>
          <div className="flex items-center gap-3 mb-3 px-1">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center text-white text-xs font-semibold shrink-0"
